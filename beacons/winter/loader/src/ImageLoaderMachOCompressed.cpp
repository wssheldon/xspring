...
#define LC_ROUTINES_COMMAND LC_ROUTINES
												  ...
												  LastLookup *last, bool runResolver)
	{
		*targetImage = NULL;
		...
		if (last != NULL)
		{
			if ((last->ordinal == libraryOrdinal) && (last->flags == symboFlags) && (last->name == symbolName))
			{
	...
	bool ImageLoaderMachOCompressed::incrementCoalIterator(CoalIterator &it)
	{
		...
		while (p < end)
		{
			...
			case BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
				...
				if (!segWriteable(immediate) && !segHasRebaseFixUps(immediate) && !segHasBindFixUps(immediate) && (!it.image->isExecutable() || it.image->isPositionIndependentExecutable()))
#else
