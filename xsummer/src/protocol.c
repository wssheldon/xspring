...
static bool ensure_capacity(ProtocolBuilder* builder, size_t additional) {
    if (!builder || builder->error) return false;
...
static bool append_data(ProtocolBuilder* builder, const char* data, size_t length) {
    if (!ensure_capacity(builder, length + 1)) return false;
...
ProtocolBuilder* protocol_builder_create(protocol_msg_type_t type) {
    ProtocolBuilder* builder = calloc(1, sizeof(ProtocolBuilder));
    ...
    builder->capacity = INITIAL_BUFFER_SIZE;
    builder->length = 0;
    builder->error = false;
    builder->type = type;
...
bool protocol_add_string(ProtocolBuilder* builder, const char* key, const char* value) {
    if (!builder || !key || !value || builder->error) return false;
    ...
    size_t total_len = key_len + escaped_len + 3; // key + ": " + escaped_value + "\n"
...
bool protocol_add_binary(ProtocolBuilder* builder, const char* key,
    ...
    if (!builder || !key || !data || builder->error) return false;
...
static char* bytes_to_hex(const unsigned char* data, size_t length, size_t* out_len) {
...
ProtocolBuilder* protocol_create_init(const char* client_id, const SystemInfo* info) {
    if (!client_id || !info) return NULL;

    ProtocolBuilder* builder = protocol_builder_create(PROTOCOL_MSG_INIT);
    if (!builder) return NULL;

    bool success =
        protocol_add_string(builder, "client_id", client_id) &&
...
ProtocolBuilder* protocol_create_error(int error_code, const char* error_message) {
    ...
    bool success =
        protocol_add_int(builder, "code", error_code) &&
