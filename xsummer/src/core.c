...
RTKContext* rtk_context_create(void) {
  ...
  ctx->autorelease_pool =
      ((id(*)(id, SEL))objc_msgSend)((id)poolClass, sel_registerName("alloc"));
  ctx->autorelease_pool = ((id(*)(id, SEL))objc_msgSend)(
      ctx->autorelease_pool, sel_registerName("init"));
...
void rtk_context_destroy(RTKContext* ctx) {
  ...
  if (ctx->autorelease_pool) {
    ((void (*)(id, SEL))objc_msgSend)(ctx->autorelease_pool,
...
RTKClass rtk_get_class(RTKContext* ctx, const char* class_name) {
  if (!ctx || !class_name) {
    if (ctx) {
      rtk_set_error(ctx, RTK_ERROR_INVALID_ARGUMENT,
                    "Invalid arguments to rtk_get_class");
    }
    return NULL;
  }
  ...
  if (!cls) {
    rtk_set_error(ctx, RTK_ERROR_CLASS_NOT_FOUND, "Class not found: %s",
                  class_name);
...
RTKInstance rtk_create_instance(RTKContext* ctx, const char* class_name) {
  ...
  if (!instance) {
    rtk_set_error(ctx, RTK_ERROR_INSTANCE_CREATION_FAILED,
                  "Failed to allocate instance of %s", class_name);
    return NULL;
  }
  ...
  if (!instance) {
    rtk_set_error(ctx, RTK_ERROR_INSTANCE_CREATION_FAILED,
                  "Failed to initialize instance of %s", class_name);
    return NULL;
  }
